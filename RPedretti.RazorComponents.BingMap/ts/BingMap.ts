...
export class BingMap {
    ...
    private readonly _notInit: { mapId: string, mapRef: any, config: Microsoft.Maps.IMapOptions }[] = [];

    private _mapLoaded = false;

    private invokeOnDotNet = async (mapId: string, e: Microsoft.Maps.IMouseEventArgs, refEventName: string): Promise<void> => {
        ...
        await this._maps.get(mapId)!!.mapRef.invokeMethodAsync(refEventName, evt);
    }

    private invokeChangeOnDotNet = async (mapId: string, eventName: string, refEventName: string): Promise<void> => {
        await this._maps.get(mapId)!!.mapRef.invokeMethodAsync(refEventName, eventName);
    }
    ...
    private initScript = (apiKey: string, mapLanguage: string): number => {
        if (!this._mapLoaded) {
    ...
    public initMaps = async (): Promise<number> => {
        for (var { mapId, mapRef, config } of this._notInit) {
            ...
            await mapRef.invokeMethodAsync('NotifyMapLoaded');   
        }

        this._notInit.length = 0;

        return 1;
    }

    public getMap = async (mapRef: any, mapId: string, config: any, apiToken: string, mapLanguage: string): Promise<number> => {
        ...
        try {
            ...
            this._notInit.push({ mapId, mapRef, config });
            this.initScript(apiToken, mapLanguage);
            return 0;
        }
    }
...
window.rpedrettiBlazorComponents.bingMap = window.rpedrettiBlazorComponents.bingMap || {};
